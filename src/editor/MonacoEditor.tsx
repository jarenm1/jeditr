...
export const Editor: React.FC<EditorProps> = ({
  ...
  useEffect(() => {
    if (editorRef.current && !editorInstance.current) {
      editorInstance.current = monaco.editor.create(editorRef.current, {
        ...
        language,
        theme:
          settings?.theme && typeof settings.theme === "string"
            ? settings.theme
            : settings?.theme && typeof settings.theme === "object"
              ? customThemeNameRef.current || "vs-dark"
              : "vs-dark",
      ...
      if (settings?.vimMode && (window as any).initVimMode) {
        const vimMode = (window as any).initVimMode(
  ...
  useEffect(() => {
    if (
      editorInstance.current &&
      editorInstance.current.getValue() !== content
    ) {
