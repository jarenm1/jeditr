...
import { githubPluginLoader, type GitHubPluginConfig } from "./githubLoader";
import {
  registerPlugin,
  registerPluginAPI,
  unregisterPlugin,
...
function isGitHubUrl(url: string): boolean {
  return (
    /^[^/]+\/[^/]+$/.test(url) || url.includes("raw.githubusercontent.com")
  );
}
 ...
 * ```
 */
export async function loadPluginsFromSettings(
...
export function unloadPlugin(name: string): boolean {
  ...
  if (worker) {
    ...
    showNotification("Plugin Manager", `Unloaded plugin "${name}"`, "info");
