...
export async function applyTextEdit(
  ...
  if (validateRange) {
    ...
    if (range.start.column < 0 || range.start.column > lines[range.start.line].length) {
      throw `Start column ${range.start.column} is out of bounds`
    }
    if (range.end.column < 0 || range.end.column > lines[range.end.line].length) {
  ...
  if (startLine === endLine) {
    ...
    if (newLines.length === 1) {
      lines.splice(startLine, endLine - startLine + 1, firstLine + newText + lastLine)
    } else {
      lines.splice(
        startLine,
        endLine - startLine + 1,
