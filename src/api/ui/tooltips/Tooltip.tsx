...
const TooltipComponent: React.FC<TooltipProps> = ({ tooltip, onPositionChange }) => {
  ...
  useEffect(() => {
    ...
    if (Math.abs(newPosition.x - adjustedPosition.x) > 5 || 
...
export const Tooltips: React.FC = () => {
  ...
  const updateTooltipPosition = useTooltipStore(state => state.updateTooltipPosition)
