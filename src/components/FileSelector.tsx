...
export const FileSelector: React.FC<FileSelectorProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      invoke<{ name: string; path: string }[]>('list_git_files')
        .then((fileList) => {
          setFiles(
            fileList.slice(0, 100).map(f => ({ label: f.name, value: f.path }))
          );
          setLoading(false);
        })
        .catch(() => {
  ...
  return (
    <PickerModal
      ...
      items={files}
      onSelect={onSelect}
      title={loading ? 'Loading files...' : error ? `Error: ${error}` : 'Select a file'}
    />
  );
};
