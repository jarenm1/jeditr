...
export const useNotificationStore = create<NotificationState>((set, get) => ({
  ...
  addNotification: (n) => {
    set((state) => {
      ...
      return { 
        ...
        focusedNotificationId: state.focusedNotificationId || newNotification.id
      };
    });
  },
    ...
    set((state) => {
      ...
      if (state.focusedNotificationId === id) {
        if (newNotifications.length === 0) {
          ...
          const currentIndex = state.notifications.findIndex(n => n.id === id);
          // Try to focus the next notification, or the previous one if we're at the end
          const nextIndex = currentIndex < newNotifications.length ? currentIndex : currentIndex - 1;
  ...
  focusNextNotification: () => {
    ...
    const currentIndex = state.notifications.findIndex(n => n.id === state.focusedNotificationId);
    if (currentIndex === -1) {
      set({ focusedNotificationId: state.notifications[0].id });
      return;
    }
  ...
  focusPreviousNotification: () => {
    ...
    if (!state.focusedNotificationId) {
      set({ focusedNotificationId: state.notifications[state.notifications.length - 1].id });
      return;
    }
    ...
    const prevIndex = currentIndex === 0 ? state.notifications.length - 1 : currentIndex - 1;
